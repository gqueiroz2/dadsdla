...
class rankingNewExport implements FromArray, WithMultipleSheets, WithTitle {
    ...
    public function sheets(): array{
    	...
    	array_push($sheets, new allNewExport($this->labels[0], $this->sheets, $this->typeExport));
    	...
    	if (isset($this->sheets['subMtx'])) {
	    	...
	    	for ($i=0; $i < sizeof($this->sheets['subMtx']); $i++) { 
	    		array_push($sheets, new newExport($this->labels[1], $this->sheets['subMtx'][$i], $this->sheets['subTotal'][$i], $this->sheets['new'][$i], $names, $this->typeExport));
	    	}	
    	}
