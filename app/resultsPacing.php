		...
		for ($b=0; $b < sizeof($brands); $b++) { 
            for ($m=0; $m < sizeof($months); $m++) { 
                if ($brands[$b][1] != 'ONL' && $brands[$b][1] != 'VIX') {
                    ...
                    $previousAdSales[$b][$m] = $this->defineValues($con, "ytd", $currency, $brands[$b][0], $months[$m][1], $region, $value,$pYear);
                }else{
                    $currentAdSales[$b][$m] = $this->defineValues($con, "ytd", $currency, $brands[$b][0], $months[$m][1], $region, $value, $year);
                    $previousAdSales[$b][$m] = $this->defineValues($con, "ytd", $currency, $brands[$b][0], $months[$m][1], $region, $value,$pYear);                    
                }        
                $currentTarget[$b][$m] = $this->defineValues($con, "plan_by_brand", $currency, $brands[$b][0], $months[$m][1], $region, $value, $year, "TARGET");
                $currentCorporate[$b][$m] = $this->defineValues($con, "plan_by_brand", $currency, $brands[$b][0], $months[$m][1], $region, $value, $year, "CORPORATE");
                $currentSAP[$b][$m] = $this->defineValues($con, "plan_by_brand", $currency, $brands[$b][0], $months[$m][1], $region, $value, $year, "ACTUAL");
                $previousSAP[$b][$m] = $this->defineValues($con, "plan_by_brand", $currency, $brands[$b][0], $months[$m][1], $region, $value, $pYear, "ACTUAL");
