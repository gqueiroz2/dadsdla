<table>
	<tr>
		<th colspan="{{sizeof($data['mtx'])}}">
			<span>
				<b>
					{{$data['region']}} - Market Ranking (BKGS) : ({{$data['currency'][0]['name']}}/{{strtoupper($data['value'])}})
				</b>
			</span>
		</th>
	</tr>
		...
		<tr>
			@for($n = 0; $n < sizeof($data['mtx']); $n++)
				@if(is_numeric($data['mtx'][$n][$m]))
					@if($data['mtx'][$n][0] == "Ranking")
						<td>{{$data['mtx'][$n][$m]}}º</td>
					@elseif($data['mtx'][$n][0] == "Var (%)" || $data['mtx'][$n][0] == "Var YTD (%)")
